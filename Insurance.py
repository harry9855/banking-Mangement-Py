...
class Insurance:
    def __init__(self, root):
        ...
        # Insuranceype
        lbl_InsuranceType = Label(labelframeleft, text="Loan Type :", font=("arial", 8, "bold"), padx=2, pady=6)
        lbl_InsuranceType.grid(row=9, column=0, sticky=W)

        combos_InsuranceType = ttk.Combobox(labelframeleft,textvariable=self.var_Insurance_Type, font=("arial", 9, "bold"), width=27,
                                      state="readonly")
        combos_InsuranceType["value"] = (
        "Life Insurance", "Veichle Insurance", "Health Insurance", "Fire Insurance")
        combos_InsuranceType.current(2)
        combos_InsuranceType.grid(row=9, column=1)



        # Insurance Amount
        lbl_Amount = Label(labelframeleft, text="Insurance Amount :", font=("arial", 8, "bold"), padx=2, pady=6)
        lbl_Amount.grid(row=11, column=0, sticky=W)

        combos_Amount = ttk.Combobox(labelframeleft,textvariable=self.var_Amount, font=("arial", 9, "bold"),
                                      width=27, state="readonly")
        combos_Amount["value"] = ("10lac", "50lac", "1cr", "10cr", "More")
        combos_Amount.current(1)
        combos_Amount.grid(row=11, column=1)
        ...
        # -----------Right Side Image------------------
        img3 = Image.open('E:\\python project\\images\\insurance.jpg')
        ...
        # -----------Right Side Image------------------
        img4 = Image.open('E:\\python project\\images\\insurance.jpg')
        ...
        combo_Search = ttk.Combobox(Table_frame, textvariable=self.search_var, font=("arial", 9, "bold"), width=27,
                                    state="readonly")
        combo_Search["value"] = ("Ref_Id", "Mobile", "Id_Number","Insurance_Type")
        ...
        self.cust_Details_Table = ttk.Treeview(details_table, columns=("Ref_Id", "Name", "Age", "DOB","Address",
                                                                       ...
                                                                       "IdNumber", "Insurance_Type", "Amount",
                                                                       "Date_Of_Issuing")
        ...
        self.cust_Details_Table.heading("Insurance_Type", text="Type Of Insurance")
        self.cust_Details_Table.heading("Amount", text="Amount of Insurance")
        ...
        self.cust_Details_Table.column("Insurance_Type", width=100)
    ...
    def save_data(self):
        if self.var_Insurance_Type.get()=="" or self.var_Amount.get()==""or self.var_Mobile.get()==""or self.var_IdNumber.get()==""or self.var_Date_Of_Issuing.get()=="":
            ...
            try:
                ...
                my_cursor.execute("insert into new_insurance values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",(
                    ...
                    self.var_IdProof.get(),self.var_IdNumber.get(),self.var_Insurance_Type.get(),self.var_Amount.get(),
                    self.var_Date_Of_Issuing.get()
    ...
    def fetch_data(self):
        ...
        my_cursor = conn.cursor()
        my_cursor.execute("select * from new_insurance")
    ...
    def get_cursor(self,event=""):
        ...
        self.var_Insurance_Type.set(row[9])
        ...
        if self.var_IdNumber.get()=="":
            ...
            try:
                conn = mysql.connector.connect(host="localhost", username="root", password="HardikJain@090104",
                                               database="banking_managementdb")
                my_cursor = conn.cursor()
                my_cursor.execute("update new_insurance set Name=%s,Age=%s,DOB=%s,Address=%s,Mobile=%s,Email=%s,"
                                  "IdProof=%s,IdNumber=%s,Insurance_Type=%s,Amount=%s,"
                                  "Date_Of_Issuing=%s where Ref_Id=%s",(
                    self.var_Name.get(), self.var_Age.get(), self.var_DOB.get(),
                    self.var_Address.get(), self.var_Mobile.get(), self.var_Email.get(),
                    self.var_IdProof.get(), self.var_IdNumber.get(), self.var_Insurance_Type.get(), self.var_Amount.get(),
                    self.var_Date_Of_Issuing.get(),self.var_Ref_Id.get()
                                                                                            ))
                conn.commit()
                self.fetch_data()
                conn.close()
    ...
    def mDelete(self):
        ...
        if mDelete>0:
            conn = mysql.connector.connect(host="localhost", username="root", password="HardikJain@090104",
                                           database="banking_managementdb")
            my_cursor = conn.cursor()
            query="delete from new_insurance where Ref_Id=%s"
        ...
        if self.txt_search.get() == "":
            ...
            try:
                conn = mysql.connector.connect(host="localhost", username="root", password="HardikJain@090104",
                                               database="banking_managementdb")
                my_cursor = conn.cursor()
                my_cursor.execute("select * from new_insurance where "+str(self.search_var.get())+" LIKE '%"+str(self.txt_search.get())+"%'")
                rows=my_cursor.fetchall()
                if len (rows)!=0:
                    self.cust_Details_Table.delete(*self.cust_Details_Table.get_children())
                    for i in rows:
